...
object UsageId {
  ...
  def build(frontUsageRequest: FrontUsageRequest) = buildId(List(
    ...
    Some(frontUsageRequest.frontUsageRecord.containerId),
    Some(frontUsageRequest.printUsageMetadata.sectionCode),
    Some(frontUsageRequest.printUsageMetadata.issueDate),
