...
class UsageApi(auth: Authentication, usageTable: UsageTable, usageGroup: UsageGroupOps, notifications: Notifications, config: UsageConfig, usageRecorder: UsageRecorder, liveContentApi: LiveContentApi,
  ...
  def setFrontUsages = auth { request => {
    val frontUsageRecordResult = request.body.validate[FrontUsageRequest]
    frontUsageRequestResult.fold(
      e => {
        respondError(BadRequest, "front-usage-request-parse-failed", JsError.toJson(e).toString)
      },
      frontUsageRecordResult: FrontUsageRequest => {
        val usageGroup = usageGroup.build(frontUsageRecordResult)
        usageRecorder.usageSubject.onNext(usageGroup)
